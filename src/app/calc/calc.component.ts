...
export class CalcComponent {
  ...
  constructor(private fb: FormBuilder, private httpService: HttpService, private modalService: ModalService) {
    this.form = this.fb.group({
      ...
      phone: ['', [Validators.required, Validators.pattern(/^89[0-9]{9}$|^\+79[0-9]{9}$/)]],
  ...
  buttonVisibilityToggle() {
    if (this.isFirstForm()) {
      document.getElementById('button-prev')?.classList.add('survey__button_hidden')
    }
    else {
      document.getElementById('button-prev')?.classList.remove('survey__button_hidden')
    }
  }
  ...
  onSubmit() {
    if (this.form.valid) {
      ...
      debt.forEach((item) => console.log(this.form.get(item)?.value))
      
      this.httpService.formSubmit(this.form.value).subscribe((value) => console.log(value))
      this.modalService.openModal('modal-1')
    }
    else {
      console.log(this.form.status)
    }
